...
"""
app/suppliers/oliveyoung.py
────────────────────────────
Sprint 7 – OliveYoung supplier client (global.oliveyoung.com).
...
_BASE_URL     = "https://global.oliveyoung.com"
_LOGIN_URL    = f"{_BASE_URL}/member/login"
_CHECKOUT_URL = f"{_BASE_URL}/order/checkout"


class OliveYoungClient(SupplierClient):
    """
    Supplier client for global.oliveyoung.com.
    ...
    def __init__(self, headless: bool = True) -> None:
        ...
        self._email    = os.getenv("OLIVEYOUNG_EMAIL", "")
    ...
    async def _playwright_create_order(self, order: "Order") -> str:
        ...
        log.info("oliveyoung.placement.start")
        ...
        async with async_playwright() as pw:
            ...
            try:
                ...
                log.info("oliveyoung.placement.success", supplier_order_id=supplier_order_id)
                return supplier_order_id
            except SupplierError:
                raise
            except Exception as exc:
                reason = f"OliveYoung placement error: {exc}"
                log.error("oliveyoung.placement.error", error=reason)
    ...
    async def _playwright_get_tracking(
        ...
        logger.info("oliveyoung.get_tracking.stub", supplier_order_id=supplier_order_id)
        return (None, None)

    # ── Page interaction helpers ──────────────────────────────────────────────

    async def _login(self, page: Any, log: Any) -> None:
        log.debug("oliveyoung.login")
    ...
    async def _add_to_cart(self, page: Any, order: "Order", log: Any) -> None:
        for item in order.line_items_json or []:
            ...
            log.debug("oliveyoung.add_to_cart", url=url)
            await page.goto(url, wait_until="networkidle")
            await page.click("button.add-to-cart, .btn-cart")
    ...
    async def _submit_order(self, page: Any, log: Any) -> str:
        await page.click("button.place-order, .btn-order")
        await page.wait_for_selector(".order-confirm", timeout=20_000)
        raw = await page.inner_text(".order-confirm .order-number")
        order_id = re.sub(r"\s+", "", raw).strip("#")
        if not order_id:
            raise SupplierError("Could not parse OliveYoung order ID.", retryable=False)
        return order_id

    async def _save_artifacts(
        self, page: Any, ref_id: str, reason: str
    ) -> None:
        artifact_dir = self._storage / "bot_failures" / "oliveyoung" / ref_id
