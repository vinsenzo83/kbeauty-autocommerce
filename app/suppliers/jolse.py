...
class JolseClient(SupplierClient):
    ...
    async def place_order(self, order_payload: dict) -> "PlacedOrder":
        ...
        if not self._email:
            logger.info(
                "jolse.place_order.stub",
                channel_order_id=channel_order_id,
            )
            return PlacedOrder(
                supplier_order_id=f"JOLSE-STUB-{channel_order_id[:8]}",
                status="placed",
                cost=order_payload.get("cost"),
                currency=order_payload.get("currency", "USD"),
                raw={"mode": "stub"},
            )
        raise NotImplementedError("Jolse real place_order requires Playwright.")
    ...
    async def get_order_status(self, supplier_order_id: str) -> "OrderStatus":
        ...
        if not self._email:
            logger.info(
                "jolse.get_order_status.stub",
                supplier_order_id=supplier_order_id,
            )
            return OrderStatus(
                supplier_order_id=supplier_order_id,
                status="shipped",
                tracking_number=f"JOLSE-TRK-{supplier_order_id[-6:]}",
                tracking_carrier="FedEx",
                raw={"mode": "stub"},
            )
        raise NotImplementedError("Jolse real get_order_status requires Playwright.")
